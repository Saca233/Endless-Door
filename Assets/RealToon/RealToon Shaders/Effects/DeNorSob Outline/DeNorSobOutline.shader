...
Shader  "Hidden/URP/RealToon/Effects/DeNorSobOutline"
{
    ...
    float EdgeDetect(float2 uv, float4 input_pos_cs)
    {
        ...
        edgeDepth = (edgeDepth > (depth0 * (_DepthThreshold * 0.01)) ? 1 : 0) - Linear01Depth(SampleSceneDepth(uv), _ZBufferParams); //cc
        ...
        float edgeSob = sob_fil((float)(_OutlineWidth / float2(_ScreenParams.r, _ScreenParams.g)) * obj_only, uv) > ((_SobelOutlineThreshold * 0.01) * SamDep(uv)) ? 1 : 0; //cc
